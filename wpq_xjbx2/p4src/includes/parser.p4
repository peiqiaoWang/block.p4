...
header etherent_t ethernet;
header intrinsic_metadata_t intrinsic_metadata;
...
parser parser_ipv4{
    extract(ipv4);
    return select(latest.fragment_offset, latest.ihl, latest.protocol) {
        IP_PROTOCOLS_IPHL_ICMP : parser_icmp;
        IP_PROTOCOLS_IPHL_TCP : parser_tcp;
        IP_PROTOCOLS_IPHL_UDP : parser_udp;
        default: ingress;
    }
} 
